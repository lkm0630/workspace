  ...
  포인터(pointer)
    ...
    포인터 사용시 주의할 점
      1. 꼭 초기화 하고 사용할 것! 
        int num;   // 일반 변수 선언 
        int *p;    // 포인터 변수 선언
        p = &num;   // (필수)포인터 변수 초기화 (num변수의 시작주소를 포인터 변수에 저장)
        *p = 100;  // 포인터변수 p에 저장된 주소에 100값을 대입(num=100 동일)
...
int main (){
    int a;    // 일반 변수 a 선언 (메모리: 10번지~13번지)
    int *pa = NULL;  // 포인터 변수 pa 선언 
    if(pa != NULL){  // 유효성체크 
    ...
    // *pa(간접참조연산자): 포인터 변수 pa에 저장된 주소에 99 값을 저장 
    //   ㄴ 포인터가 가리키는 위치의 값을 추출 
    //   ㄴ () 다음으로 높은 우선순위르 가짐 → 다른 연산자보다 먼저 실행됨 
    *pa = 99; 
